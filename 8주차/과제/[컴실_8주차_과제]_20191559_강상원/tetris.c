...
int modified_recommend(NodePTR root) {
    ...
    int rotPossible[7] = {2, 4, 4, 4, 1, 2, 2}; //블럭의 종류마다 가능한 회전의 수

    size_space += sizeof(*root); //공간복잡도 계산
...
void AutoDown(int sig) {
    if (!CheckToMove(field, nextBlock[0], blockRotate, blockY, blockX)) gameOver = 1;
        ...
        score += AddBlockToField(field, nextBlock[0], R_rec, Y_rec, X_rec);
        score += DeleteLine(field);
        ...
        DrawBlockWithFeatures(blockY, blockX, nextBlock[0], blockRotate); //asfads
        timed_out = 0;
    }
}
...
void recommendedPlay() {
    ...
    do {
        ...
        ////시간, 공간 출력
        END_time=time(NULL);
        double duration = (double)difftime(END_time, START_time);

        move(3, WIDTH + 25);
        printw("time            : %10.2f   s", duration);
        move(4, WIDTH + 25);
        printw("score per time  : %10.2f / s", score / duration);
        move(6, WIDTH + 25);
        printw("space           : %10.2f  KB", (double)size_space/1024);
        move(7, WIDTH + 25);
        printw("space per time  : %10.2f  KB / s", (double)size_space / duration / 1024);
        move(9, WIDTH + 25);
        printw("score per space : %10.2f / KB", score / ((double)size_space/1024));
        ////
    } while (!gameOver);
